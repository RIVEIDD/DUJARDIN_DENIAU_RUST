...
pub fn start_game(player1: &mut Player, player2: &mut Player, n_objectives: usize) {
    ...
    loop {
        ...
        let score1 = play_turn(player1, n_objectives).unwrap_or(0);
        let score2 = play_turn(player2, n_objectives).unwrap_or(0);
...
fn play_turn(player: &Player, n_objectives: usize) -> Result<u32, std::io::Error> {
    ...
    while (wait_for_key()?).is_some() {
        // continue tant que ce n'est pas None (c'est-à-dire tant que ce n'est pas Entrée)
    }
    ...
    let _ = *compteur.lock().unwrap();
    let _ = *miss.lock().unwrap();

    let mut total_score = 0;

    for (&cible, &valeur) in &objectives {
        println!("→ Objectif {}:{} :", cible, valeur);
    ...
    let moyenne = total_score / objectives.len().max(1) as u32;
    println!("→ Score moyen {}", moyenne);
    Ok(moyenne)
}